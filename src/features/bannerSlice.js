  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(getBannerById.fulfilled, (state, action) => {
        state.status = "succeeded";
        // Assuming the payload contains the banner data
        state.banners = [action.payload.banner]; // Update the state with the fetched banner
      })
      .addCase(getBannerById.rejected, (state, action) => {
        state.status = "failed";
        state.error = action.error.message;
      })
      // Handling for getAllBannerImages
